...
function sumEvenArguments(){
    ...
    return newArr.reduce(function(accumulator, nextValue, index, array){
        if(nextValue % 2 === 0){
...
/*
Write a function called once which accepts two parameters, a function and a value for the keyword 'this'. => this를 받아서 뭐에 쓰려고?
...
Examples:
    ...
    var addOnce = once(add, this); => function을 return 함. 한 번만 invoke 될 수 있음.
    addOnce(2,2) // 4
...
function once(fn, thisArg){
    ...
    return function(){
        if(!isInvoked){
            isInvoked = true;
            var newArr = [].slice.call(arguments);
            return fn.apply(thisArg, newArr);
        }
    }
}
...
// 1 - function which accepts a function and a value for the kyeword this
function bind (fn, this){
    return function new(){
        var anonymous = fn.bind();
        invokeAnother()
    }
}
...
function bind(fn, thisArg){
    var outSideArguments = arguments;
    var newFunc = fn.bind(thisArg);
    return function innerBind(){
        var newArr = [].slice.call(arguments);
        // console.log("This is outside Arguments: ", outSideArguments);
        // console.log("This is outside Arguments Length: ", outSideArguments.length);
        for(var i = 2; i < outSideArguments.length; i++){
            newArr.push(outSideArguments[i]);
        }

        return newFunc.apply(thisArg, newArr);
    }
}
...
var bindFn2 = bind(firstNameFavoriteColor, person, 'blue');
console.log(bindFn2('blue')); // "Elie's favorite color is blue"
