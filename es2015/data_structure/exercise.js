...
function hasDuplicates(){
    var arr = arguments[0];
    console.log(arr);
    var checkSetSize = new Set;

    arr.forEach(element => {
        checkSetSize.add(element);
    });

    console.log("array-length:", arr.length);
    console.log("set-size:",checkSetSize.size);

    if(Number(arr.length) === Number(checkSetSize.size)){
        return false;
    } else {
        return true;
    }
...
function countPairs(){
    let pairCounter = 0;
    let arr = arguments[0];
    let standard = arguments[1];

    for(let i = 0; i < arr.length; i++){
        for(let j = i + 1; j < arr.length; j++){
            if(arr[i] + arr[j] === standard){
                pairCounter++;
            }
        }
    }
